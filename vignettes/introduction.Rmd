...
```{r data_loading}
# Enhanced city filtering - Madrid example
...
## Enhanced Data Loading and Multi-City Capabilities
...
# Demonstrate multi-city analysis capabilities using enhanced get_zones function
cat("=== Multi-City Comparison Analysis ===\n")

# Safely load additional Spanish cities using enhanced city filtering
...
city_comparison_plot <- ggplot(city_comparison_long, aes(x = city, y = value, fill = city)) +
  ...
  labs(title = "Spanish Cities Comparative Analysis",
       ...
       x = "City", y = "Value",
       caption = "Enhanced mobspain package enables direct city name filtering") +
  theme_minimal() +
  theme(
...
# Create comprehensive three-city hourly comparison
multi_city_plot <- ggplot(multi_city_hourly, aes(x = hour, y = avg_trips, color = city)) +
  geom_line(size = 2.2, alpha = 0.9) +
  ...
  scale_color_manual(values = c("Madrid" = "#E74C3C", "Barcelona" = "#3498DB", "Valencia" = "#27AE60"),
                     name = "City") +
  scale_x_continuous(breaks = seq(6, 22, 4)) +
  scale_y_continuous(labels = scales::comma_format()) +
  labs(title = "Multi-City Hourly Transport Demand Comparison",
       subtitle = "Distinct mobility patterns across Spain's major metropolitan areas",
       x = "Hour of Day", y = "Average Trips per Route",
       caption = "Comparing Madrid (capital), Barcelona (tourism/business), Valencia (regional center)") +
  theme_minimal() +
  theme(
    legend.position = "top",
...
# Demonstrate enhanced package capabilities
cat("\n=== Enhanced Package Functionality Demo ===\n")
...
cat("- Enhanced accessibility for transport research\n")
...
```{r demand_assessment}
# Examine transport demand patterns using mobspain functionality
cat("=== Transport Demand Analysis ===\n")

cat("Total mobility records:", nrow(madrid_mobility), "\n")
cat("Unique origin zones:", length(unique(madrid_mobility$origin)), "\n")
cat("Unique destination zones:", length(unique(madrid_mobility$dest)), "\n")
cat("Total passenger trips:", sum(madrid_mobility$n_trips), "\n")
cat("Analysis period:", min(madrid_mobility$date), "to", max(madrid_mobility$date), "\n")

# Peak vs off-peak demand analysis
peak_summary <- madrid_mobility %>%
  group_by(is_peak) %>%
  summarise(
    total_trips = sum(n_trips),
...
weekend_summary <- madrid_mobility %>%
  group_by(is_weekend) %>%
  summarise(
    total_trips = sum(n_trips),
...
hourly_pattern <- madrid_mobility %>%
...
ggplot(hourly_pattern, aes(x = hour, y = avg_trips, color = is_weekend)) +
  ...
  scale_y_continuous(labels = scales::comma_format()) +
  labs(title = "Madrid Hourly Transport Demand Patterns",
       subtitle = "Peak commuting hours and weekend leisure travel patterns",
       x = "Hour of Day", y = "Average Trips per Route",
       color = "Period") +
  theme_minimal() +
  theme(
...
# Join with spatial data for mapping using enhanced get_zones output
...
madrid_origin_map <- ggplot() +
  # Add all Madrid zones as base layer for proper context
  geom_sf(data = madrid_zones, fill = "#F8F9FA", color = "white", alpha = 0.3, size = 0.2) +
  ...
  labs(title = "Madrid Transport Origin Hotspots",
       subtitle = "Major departure points across Madrid metropolitan area",
       caption = "Source: Enhanced mobspain analysis with city filtering") +
  theme_void() +
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5, color = "#2C3E50"),
    plot.subtitle = element_text(size = 12, color = "#7F8C8D", hjust = 0.5, margin = margin(b = 15)),
...
madrid_dest_map <- ggplot() +
  ...
  labs(title = "Madrid Transport Destination Hotspots",
...
## Spatial Optimization and Network Analysis

```{r spatial_optimization}
# Advanced spatial analysis using enhanced mobspain features
cat("=== Spatial Network Optimization Analysis ===\n")

# Identify major transport corridors for high-capacity planning
...
# Calculate spatial characteristics using Madrid zone coordinates
...
# Enhanced flow analysis with spatial metrics
...
# Hub identification using enhanced scoring
hub_analysis <- network_stats %>%
  mutate(
    # Enhanced hub scoring considering connectivity and demand
...
# Create enhanced flow map for major corridors within Madrid
if(nrow(flow_coords) > 0) {
  flow_map <- ggplot() +
    # Madrid zones as base layer
    geom_sf(data = madrid_zones, fill = "#F8F9FA", color = "white", alpha = 0.4, size = 0.1) +
    # Madrid boundary for context
    geom_sf(data = madrid_boundary, fill = NA, color = "#2C3E50", size = 0.8) +
    # Major transport flows as curved paths
    geom_curve(data = flow_coords,
               aes(x = origin_lon, y = origin_lat, 
                   xend = dest_lon, yend = dest_lat,
                   color = flow_category, size = flow_category),
               alpha = 0.8, curvature = 0.15, ncp = 5) +
    # Origin points highlighted
    geom_point(data = flow_coords,
               aes(x = origin_lon, y = origin_lat),
               color = "#E74C3C", alpha = 0.9, size = 3.5, shape = 21, 
               fill = "#FFFFFF", stroke = 1.2) +
    # Destination points highlighted  
    geom_point(data = flow_coords,
               aes(x = dest_lon, y = dest_lat),
               color = "#3498DB", alpha = 0.9, size = 3.5, shape = 24, 
               fill = "#FFFFFF", stroke = 1.2) +
    ...
    scale_size_manual(name = "Flow\nIntensity",
                      values = c("Low" = 1, "Medium" = 1.8, "High" = 2.5, "Very High" = 3.2),
                      guide = "none") +
    labs(title = "Madrid Major Transport Corridors",
...
madrid_map <- ggplot(madrid_categorized) +
  ...
  labs(
    ...
    caption = "Data: mobspain package | Enhanced city filtering capability"
  )
...
cat("=== ENHANCED MOBSPAIN CAPABILITIES ===\n\n")
...
cat("ENHANCED APPROACH (Current Implementation):\n")
...
cat("   - Enhanced package accessibility through city name filtering\n")
